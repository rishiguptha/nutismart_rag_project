...
# Update to import the streaming function
from .llm_client import stream_llm_response # <--- Import the streaming version
...
DEFAULT_TOP_K = 5 # Maybe increase default slightly
...
# --- Main RAG Query Function - Now returns a Generator ---
def query_rag_stream(user_query: str, top_k: int = DEFAULT_TOP_K) -> Generator[str, None, None]:
    ...
    prompt = format_react_style_rag_prompt(user_query, context_chunks)

    logger.info("Streaming prompt to external LLM...")
    # Call the streaming LLM function and yield its results directly
    response_generator = stream_llm_response(prompt)

    # We don't need to extract the final answer here anymore,
    # the UI will consume the stream directly.
    # However, we should handle the case where the generator is None (init failed)
    if response_generator is None:
        yield "Sorry, the Language Model client is not available."
        return

    yield from response_generator # Yield chunks from the LLM stream

# --- Main Execution Block (Command-line version - Doesn't show streaming well) ---
# This block is less useful now as streaming needs a UI to be seen effectively.
# You can keep it for basic testing if needed, but it will print chunks sequentially.
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            print("\nStreaming Answer:")
            full_response = ""
            # Iterate through the generator returned by query_rag_stream
